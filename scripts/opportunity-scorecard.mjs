...
const { appendOpportunityScorecardReceipt, opportunityScorecardPlan } = require("../data/opportunity-scorecard");
...
async function main() {
  ...
  try {
    ...
    const receipt = appendOpportunityScorecardReceipt(root, {
      id: `opportunity-scorecard-${Date.now().toString(36)}`,
      ...
      topScorecards: report.scorecards.slice(0, 8).map((item) => ({
        id: item.id,
        rank: item.scoreRank,
        overallScore: item.overallScore,
        band: item.band,
        manualUseGate: item.decision.manualUseGate,
        residualRisk: item.risk.residualRisk,
      })),
    ...
    console.log(
      `${receipt.id} ${receipt.summary.passing}/${receipt.summary.checks} check(s) passed; ${receipt.summary.scorecards} scorecard(s); top ${receipt.summary.topOpportunityId}; score ${receipt.summary.score}/100; wrote ${plan.receiptStore}`,
    );
