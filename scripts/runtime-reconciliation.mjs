...
const { appendRuntimeReconciliationReceipt, runtimeReconciliationPlan } = require("../data/runtime-reconciliation");
...
async function main() {
  ...
  try {
    ...
    const receipt = appendRuntimeReconciliationReceipt(root, {
      id: `runtime-reconciliation-${Date.now().toString(36)}`,
      ...
      routeLedger: report.routeLedger,
    ...
    console.log(
      `${receipt.id} ${receipt.summary.passing}/${receipt.summary.checks} check(s) passed; ${receipt.summary.receiptKinds} receipt kind(s); stale ${receipt.summary.staleReceiptKinds}; score ${receipt.summary.score}/100; wrote ${plan.receiptStore}`,
    );
