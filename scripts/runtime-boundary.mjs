...
const { appendRuntimeBoundaryReceipt, runtimeBoundaryPlan } = require("../data/runtime-boundary");
...
async function main() {
  ...
  try {
    ...
    const receipt = appendRuntimeBoundaryReceipt(root, {
      id: `runtime-boundary-${Date.now().toString(36)}`,
    ...
    console.log(
      `${receipt.id} ${receipt.summary.passing}/${receipt.summary.checks} check(s) passed; public ${receipt.summary.publicApiRoutes}; private ${receipt.summary.privateApiRoutes}; score ${receipt.summary.score}/100; wrote ${plan.receiptStore}`,
    );
