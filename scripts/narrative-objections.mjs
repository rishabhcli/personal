...
const { appendNarrativeObjectionReceipt, narrativeObjectionPlan } = require("../data/narrative-objections");
...
async function main() {
  ...
  try {
    ...
    const receipt = appendNarrativeObjectionReceipt(root, {
      id: `narrative-objections-${Date.now().toString(36)}`,
    ...
    console.log(
      `${receipt.id} ${receipt.summary.passing}/${receipt.summary.checks} check(s) passed; ${receipt.summary.audiences} audience(s); ${receipt.summary.objections} objection(s); score ${receipt.summary.score}/100; wrote ${plan.receiptStore}`,
    );
