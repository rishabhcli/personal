...
const { appendRuntimeDeployReadinessReceipt, runtimeDeployReadinessPlan } = require("../data/runtime-deploy-readiness");
...
async function main() {
  ...
  try {
    ...
    const receipt = appendRuntimeDeployReadinessReceipt(root, {
      id: `runtime-deploy-readiness-${Date.now().toString(36)}`,
      ...
      gates: report.gates,
      comparisonPacket: report.comparisonPacket,
      handoffChecklist: report.handoffChecklist,
    ...
    console.log(
      `${receipt.id} ${receipt.summary.passing}/${receipt.summary.checks} check(s) passed; ${receipt.summary.passingGates}/${receipt.summary.gates} gate(s); score ${receipt.summary.score}/100; wrote ${plan.receiptStore}`,
    );
