...
const { appendGraphDepthScoreReceipt, graphDepthScorePlan } = require("../data/graph-depth-score");
...
async function main() {
  ...
  try {
    ...
    const receipt = appendGraphDepthScoreReceipt(root, {
      id: `graph-depth-score-${Date.now().toString(36)}`,
      ...
      depthSummaries: report.depthSummaries.map((summary) => ({
        id: summary.id,
