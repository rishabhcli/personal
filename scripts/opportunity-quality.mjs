...
const { appendOpportunityQualityReceipt, opportunityQualityPlan } = require("../data/opportunity-quality");
...
async function main() {
  ...
  try {
    ...
    const receipt = appendOpportunityQualityReceipt(root, {
      id: `opportunity-quality-${Date.now().toString(36)}`,
    ...
    console.log(
      `${receipt.id} ${receipt.summary.passing}/${receipt.summary.checks} check(s) passed; ${receipt.summary.packages} package(s); score ${receipt.summary.score}/100; wrote ${plan.receiptStore}`,
    );
