...
const { appendEvaluationIntegrityReceipt, evaluationIntegrityPlan } = require("../data/evaluation-integrity");
...
async function main() {
  ...
  try {
    ...
    const receipt = appendEvaluationIntegrityReceipt(root, {
      id: `evaluation-integrity-${Date.now().toString(36)}`,
    ...
    console.log(
      `${receipt.id} ${receipt.summary.passing}/${receipt.summary.checks} check(s) passed; ${receipt.summary.passingDomains}/${receipt.summary.domains} domain(s); score ${receipt.summary.score}/100; wrote ${plan.receiptStore}`,
    );
