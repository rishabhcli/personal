...
const { appendEvaluationSampleReceipt, evaluationSamplePlan } = require("../data/evaluation-sample");
...
async function main() {
  ...
  try {
    ...
    const receipt = appendEvaluationSampleReceipt(root, {
      id: `evaluation-sample-${Date.now().toString(36)}`,
    ...
    console.log(
      `${receipt.id} ${receipt.summary.passing}/${receipt.summary.samples} sample(s) passed; ${receipt.summary.domains} domain(s); score ${receipt.summary.score}/100; wrote ${plan.receiptStore}`,
    );
