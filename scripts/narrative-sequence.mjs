...
const { appendNarrativeSequenceReceipt, narrativeSequencePlan } = require("../data/narrative-sequence");
...
async function main() {
  ...
  try {
    ...
    const receipt = appendNarrativeSequenceReceipt(root, {
      id: `narrative-sequence-${Date.now().toString(36)}`,
      ...
      sequences: report.sequences.map((sequence) => ({
        id: sequence.id,
        score: sequence.score,
        band: sequence.band,
        riskLevel: sequence.riskLevel,
        beatCount: sequence.beatCount,
        failingChecks: sequence.checks.filter((check) => !check.passed).map((check) => check.id),
      })),
      ...
      sequenceContract: report.sequenceContract,
    ...
    console.log(
      `${receipt.id} ${receipt.summary.passing}/${receipt.summary.checks} check(s) passed; ${receipt.summary.audiences} audience(s); ${receipt.summary.totalBeats} beat(s); score ${receipt.summary.score}/100; wrote ${plan.receiptStore}`,
    );
