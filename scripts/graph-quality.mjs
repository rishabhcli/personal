...
const { appendGraphQualityReceipt, graphQualityPlan } = require("../data/graph-quality");
...
async function main() {
  ...
  try {
    ...
    const receipt = appendGraphQualityReceipt(root, {
      id: `graph-quality-${Date.now().toString(36)}`,
    ...
    console.log(
      `${receipt.id} ${receipt.summary.passing}/${receipt.summary.checks} check(s) passed; nodes ${receipt.summary.nodes}; edges ${receipt.summary.edges}; audit ${receipt.summary.auditCoverageScore}/100; wrote ${plan.receiptStore}`,
    );
