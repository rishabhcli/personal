...
describe("personal command center API", () => {
  ...
  it("serves the named public API surfaces", async () => {
    ...
    assert.ok(Buffer.byteLength(JSON.stringify(evaluationSample.body)) < 2500);
    ...
    assert.ok(evaluationSample.body.samples.every((sample) => sample.detailAvailable && sample.verificationCommandAvailable));
    assert.ok(evaluationSample.body.nonClaims.some((item) => /analytics|keystrokes|private cockpit/i.test(item)));
